...
function ProfileAvatar({
  ...
  const size = mobile ? "h-[19px] w-[19px]" : "h-[22px] w-[22px]";
  ...
  if (user.profileImage) {
    return (
      <span
        className={`relative ${size} overflow-hidden rounded-full border border-black/10 bg-[#e8e4dc]`}
      >
        <Image
          src={user.profileImage}
          alt={user.name}
          fill
          sizes={mobile ? "19px" : "22px"}
  ...
  /* Name initial if no DP uploaded */
  const initial = user.name?.trim().charAt(0).toUpperCase() || "U";

  return (
    <span
      className={`flex ${size} items-center justify-center rounded-full bg-[#121212] text-[10px] font-semibold text-[#D4C29A]`}
    >
      {initial}
    </span>
  );
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    /*
      This allows the Header to update automatically
      when the Account page changes the profile.
    */
    window.addEventListener("hs-fabrics-user-updated", loadUser);
    window.addEventListener("storage", loadUser);

    return () => {
      window.removeEventListener(
        "hs-fabrics-user-updated",
        loadUser
      );
  ...
  return (
    <>
      {/* Announcement bar */}
      <div className="relative z-[60] bg-black px-4 py-2 text-center text-[9px] font-medium uppercase tracking-[0.25em] text-white sm:text-[10px]">
      ...
      <header className="sticky top-0 z-50 border-b border-black/[0.07] bg-[#f8f7f3]/95 backdrop-blur-xl">
        <div className="mx-auto flex h-[68px] max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:h-[76px] lg:px-10">
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-2">
            ...
            <Link
              href="/account"
              aria-label={user ? `Profile of ${user.name}` : "Account"}
        ...
        >
          ...
          <div className="border-t border-black/[0.07] px-5 py-4">
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                <ProfileAvatar user={user} mobile />
